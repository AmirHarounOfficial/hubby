...
export default function NewCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(t('categories.createFailed'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{t('categories.createTitle')}</h1>
          <p className="text-sm text-muted-foreground">{t('categories.createSubtitle')}</p>
        </div>
      </div>
